...
pub fn render(world: Box<dyn Model + Sync>, camera: Box<Camera>, width: usize, height: usize, samples: usize) -> Arc<Mutex<Vec<Vec<Pixel>>>> {
    let mut frame = Arc::new(Mutex::new(vec![Vec::with_capacity(5); height]));
